...
class Polygon {
  ...
  public static boolean collided; //placeHolder checking for collisions;
  public static boolean collidedApple;
...
//    for (Point p : shape) {
  ...
  public void respawn() {
		/*
		 * if a snake collides with an apple, it will respawn at a random position
		 * it is the same object just at a new location. 
		 */
  ...
  public boolean collides(Polygon other) {
	  ...
	  for(Point point : other.getPoints()) {
		  checker = point;
		  if(this.contains(checker) && (other instanceof Fruit)) {
			  collidedApple = true;
			  return true;
		  }
		  
		  if(this.contains(checker) && (other instanceof PowerUps)){
			  collided = true;
			  return true;
		  }
	  ...
	  Point checker2;
	  for(Point point2 : this.getPoints()) {
		  checker2 = point2;
		  if(other.contains(checker2)) {
